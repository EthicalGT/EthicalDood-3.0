...
import java.io.*;
import java.sql.*;
class user_files extends JFrame implements ActionListener{
    ...
    user_files(){
        ...
        InputStream font=user_files.class.getResourceAsStream("font/Poppins-Regular.ttf");
        Font myfont=null;
        try{
        ...
        try{
            ...
            ResultSet op=state.executeQuery("select * from user_files");
            while(op.next()){
            mname=op.getString("mname");
            fname=op.getString("filename");
            fdesc=op.getString("filedesc");
            String fpath=op.getString("filepath");
            JLabel lbl=new JLabel(mname);
            lbl.setBounds(i,j,150,50);
            lbl.setFont(myboldfont.deriveFont(12f));
            panel.add(lbl);
        ...
        panel.add(creator);
        JLabel tagline=new JLabel("Member Arrivals");
        tagline.setFont(myboldfont.deriveFont(70f));
        tagline.setForeground(Color.WHITE);
        tagline.setBounds(410,100,600,100);
        panel.add(tagline);
        JLabel p1=new JLabel("<html>--| All the ED Members's who upload thier codes will be display over here, Encourage Members to do more for you too..! |--</html>");
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new user_files());
